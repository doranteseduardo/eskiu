...
- *`std::variant` + `std::visit`* — Using `std::variant<BinaryExpr, CallExpr, ...>` for the node union gives exhaustive static dispatch. Rejected because the AST hierarchy is deep (26 concrete node types across three inheritance branches), variant nesting becomes unwieldy, and `shared_ptr` polymorphism is needed because nodes can be referenced from multiple places (e.g. `expressionTypes` in the type checker holds raw `Expr*` keys).
...
**Why visitor:** Adding a new pass means implementing a new `ASTVisitor` subclass. Existing AST nodes and existing passes are not touched. The compiler has three pass implementations today (`ASTPrinter`, `TypeChecker`, `CodeGen`); each was added without modifying the other two. The `ASTVisitor` base class declares 26 pure-virtual `visit()` overloads — one per concrete node type — so the compiler enforces coverage: a new node type added to `ast.h` that lacks a `visit()` overload causes a compile-time error in all three passes simultaneously.
...
**Decision:** Error propagation in Eskiu is done via `Result<T,E>` values (`struct Result<T,E> { int ok; T value; E error; }`). Exceptions are a planned Phase 10 feature. In v0.1, the decoder checks `result.ok` and branches manually.
