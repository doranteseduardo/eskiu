...
- [~] `async` / `await` — **frontend done**; **transform: single + multiple awaits working** (`sema/async_transform.cpp` lowers an async fn to a frame + N+1-state resume if-chain + constructor; validated fast-path and single/multi suspend-over-reactor end-to-end). **Remaining:** await in expression position / `return await`, `async void`, control flow around await, implicit `future_drop`, freeing waker envs. Design locked in `docs/dev/async-design.md`.
