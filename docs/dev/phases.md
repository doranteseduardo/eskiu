...
- [~] `async` / `await` — **frontend done**; **transform: single + multiple awaits working** (`sema/async_transform.cpp` lowers an async fn to a frame + N+1-state resume if-chain + constructor; validated fast-path and single/multi suspend-over-reactor end-to-end). **Leak-free** (verified with `leaks`: 0 leaks on every async path + executor/net_async). **Remaining:** control flow around await (if/while containing awaits). (`return await`, bare `await`, `async void`, cancellation, and full closure-env ownership now done.) Design locked in `docs/dev/async-design.md`.
