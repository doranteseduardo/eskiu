...
- [~] **Sum types (algebraic enums + `match`)** — payload-bearing `enum` variants make the enum a tagged union; variants are constructed by name and destructured with `match` (binds payload fields per arm). **S1 done** (concrete payload types, construction, match, `_` default; classic int enums unchanged; test `enum_adt`). Remaining: S2 exhaustiveness checking, S3 generic enums (`Option<T>`/`Either<A,B>`).
