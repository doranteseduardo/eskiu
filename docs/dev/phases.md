...
- [~] **Sum types (algebraic enums + `match`)** — payload-bearing `enum` variants make the enum a tagged union; variants are constructed by name and destructured with `match` (binds payload fields per arm). **S1 + S2 done** — concrete payload types, construction, match with `_` default, **exhaustiveness checking** (missing-variant / duplicate-arm errors); classic int enums unchanged. Tests: `enum_adt`, `errors/match_nonexhaustive`. Remaining: S3 generic enums (`Option<T>`/`Either<A,B>`).
- [ ] **Generic value-returning `select`/`join`** — `select2<A,B>` returning the winner's *value* (not just its index) and `join2<A,B>` returning both values as a pair. **Blocked on sum types S3** (generic `Either<A,B>`): once generic algebraic enums land, `select` can return "an A or a B" type-safely.
