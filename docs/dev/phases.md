...
- [~] `async` / `await` — **frontend done**; **transform: single + multiple awaits working** (`sema/async_transform.cpp` lowers an async fn to a frame + N+1-state resume if-chain + constructor; validated fast-path and single/multi suspend-over-reactor end-to-end). **Remaining:** control flow around await, and a closure-env-ownership pass to close bounded per-operation leaks. (`return await`, bare `await`, `async void`, and cancellation via `future_drop` cascade now supported; resume-waker envs freed.) Design locked in `docs/dev/async-design.md`.
