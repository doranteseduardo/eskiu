...
- [~] `async` / `await` — **frontend done**; **transform: single + multiple awaits working** (`sema/async_transform.cpp` lowers an async fn to a frame + N+1-state resume if-chain + constructor; validated fast-path and single/multi suspend-over-reactor end-to-end). **Remaining:** await inside a larger expression, control flow around await, implicit `future_drop` + freeing waker envs. (`return await`, bare `await`, and `async void` now supported.) Design locked in `docs/dev/async-design.md`.
