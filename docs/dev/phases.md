...
- [ ] **Async polish** (builds directly on the finished async stack):
  - [ ] **Concurrent `<http_async>`** — `spawn` each accepted connection's handler so the accept loop serves connections concurrently instead of sequentially (the limitation noted above). **Depends on a completion primitive** (a wait-group): once handlers are detached, the accept loop finishing no longer means the in-flight work is done, so clean bounded shutdown needs to await all outstanding handlers — which falls out of **channels** below. So: channels first, then this.
  - [ ] **Async examples** — showcase `select2`/`join2`/`spawn`/`timer_after` in `examples/` (today only a basic suspend example exists).
  - [ ] **Tighter locals-across-await liveness** — only hoist a local to the frame struct if it is live across an `await`; non-crossing locals can stay stack temporaries (smaller frames).
- [ ] **Channels** — an async, bounded message channel integrated with the `Future`/executor model: `chan_recv(ch)` is a `*Future<T>` that completes when an item is available, `chan_send(ch, v)` enqueues and wakes a waiting receiver. The inter-task communication primitive the async-design roadmap names as the next combinator-shape; foundation for the future UI framework's event flow.
- [ ] **Generic value-returning `select`/`join`** — `select2<A,B>` returning the winner's *value* (not just its index) and `join2<A,B>` returning both values as a pair. **Blocked on sum types** (`Either<A,B>` / a tagged union): without them `select` cannot return "an A or a B" type-safely, so the index-returning form stays the design until sum types land.
