...
- [ ] **Reinforcement candidate: exhaustiveness for classic (payload-less) enums.** `match` is exhaustive but ADT-only (it rejects a plain `enum { V1, V2 }`: "match subject must be an algebraic enum, got int"), and `switch` accepts a plain enum but is NOT exhaustiveness-checked (omitting a case compiles silently). So the "add a variant and every unhandled site becomes a compile error" guarantee, the main safety win of enums, is unavailable for the simple state/format tags plain enums are for. Surfaced while dogfooding the ine-qr decoder (a `QRFormat { V1, V2 }` dispatch that could not be matched, only `if`-chained). Close it by either letting `match` accept a plain enum (exhaustive over its variant set) or making a `switch` over an enum subject exhaustiveness-checked. Reinforcement (closes a gap between two existing constructs), not new surface.
