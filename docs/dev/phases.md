...
- [~] `async` / `await` — **frontend done**; **transform: single `await` working** (`sema/async_transform.cpp` lowers an async fn to a frame + resume if-chain + constructor; validated fast-path and suspend-over-reactor end-to-end). **Remaining:** multiple awaits, await in expression position, `async void`, tighter locals-across-await, implicit `future_drop`, freeing waker envs. Design locked in `docs/dev/async-design.md`.
