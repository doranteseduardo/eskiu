...
- [ ] **Async polish** (builds directly on the finished async stack):
  - [ ] **Concurrent `<http_async>`** — `spawn` each accepted connection's handler so the accept loop serves connections concurrently instead of sequentially (the limitation noted above).
