...
Dogfooding the compiler in Eskiu found bugs the fuzzer couldn't — that's partly the point
...
- **`alloc` doesn't zero; never read a field a node kind doesn't set.** Several AST walkers
  ...
  (`MallocPreScribble=1 MALLOC_PROTECT_BEFORE=1` fills fresh allocations with `0xAA` → a
