...
- **`async` / `await`** — `async T f(...)` whose call yields `*Future<T>`, and the `await E` expression (`E: *Future<T>` → `T`), legal only inside an async fn. A pre-codegen pass lowers an async function to a resumable state machine (frame struct + resume + constructor). Single-`await` functions run end-to-end (fast path and suspend-over-reactor); multi-await, `async void`, and a few other shapes are still in progress.
