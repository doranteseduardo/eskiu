...
- **Algebraic enums + `match`** — an `enum` variant may carry a payload (`enum Shape { Circle(float), Rect(float, float), Unit }`), making the enum a tagged union. Variants are constructed by name (`Circle(2.0)`, bare `Unit`) and destructured with `match s { Circle(r) -> …; _ -> … }`, which binds payload fields per arm. Classic integer enums are unchanged. `match` is checked for **exhaustiveness** (every variant covered, or a `_` default) and rejects duplicate arms. (Generic algebraic enums — `Option<T>`/`Either<A,B>` — to follow.)
