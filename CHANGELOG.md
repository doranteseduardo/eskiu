...
### Standard library
- **`<http2>`** — HTTP/2 (RFC 7540), stage 1: the frame-header codec (`h2_write_header`/`h2_read_header` — big-endian length/type/flags/31-bit stream id), frame-type + flag constants (`H2_HEADERS`, `H2_SETTINGS`, …), and the connection preface. Foundation for the staged build (HPACK, streams + flow control, TLS/ALPN, server API — see `docs/dev/http2-design.md`).
