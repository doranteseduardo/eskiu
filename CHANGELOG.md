...
- **`async` / `await` (frontend)** — `async T f(...)` whose call yields `*Future<T>`, and the `await E` expression (`E: *Future<T>` → `T`), legal only inside an async fn. Parsing + type-checking; the state-machine lowering is in progress.
