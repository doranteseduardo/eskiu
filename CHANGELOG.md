...
- **`<hpack>`** — HPACK header compression (RFC 7541), stage 3a: prefix-integer coding (§5.1), raw string literals (§5.2), the 61-entry static table, a dynamic table with size-based eviction, and the §6 decoder/encoder (indexed; literal with/without/never indexing; dynamic table size update). Verified against the RFC's own vectors (§C.1.1, §C.3.1) plus an encode→decode round-trip (`hpack`). Huffman coding (§5.2 / Appendix B) is the remaining stage-3b piece. See `docs/dev/http2-design.md` for the staged build (streams + flow control, TLS/ALPN, server API still ahead).
