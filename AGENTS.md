...
| `async` / `await` (frontend) | `async` is a `FunctionDecl` modifier (`isAsync`); the call types as `*Future<T>` while the body returns the inner `T`. `await E` is `AwaitExpr` — requires `E: *Future<T>`, yields `T`, legal only inside an async fn (`inAsyncFn` in the type checker). **Lowering (AST→state-machine transform) is not yet implemented** — codegen of `AwaitExpr` throws; testable via `--test-parser`/`--test-typechecker`. Runtime it targets: `<future>`/`<executor>`/`<net_async>`. See `docs/dev/async-design.md` |
