...
| labeled `break` / `continue` | A loop can carry a leading label (`outer: for (...)`); `break outer` / `continue outer` target that loop from a nested one. Label lives on the loop node (`ForStmt`/`WhileStmt`/`DoWhileStmt`/`ForInStmt` `.label`) and the jump node (`BreakStmt`/`ContinueStmt` `.label`; `""` = unlabeled). Parser detects `IDENT ':'` before a loop keyword. Sema keeps a `loopLabelStack` (self-host `loop_lbls`) and rejects an unknown label; the defer-escape guard also rejects any labeled jump inside a `defer`. Codegen keeps a **loops-only** frame stack (`loopStack` C++ / `loop_lbl*` self-host, separate from the shared break-target stack a `switch` also pushes to) of `{label, breakBlock, continueBlock, cleanupDepth}`; a labeled jump scans it innermost-first, runs `runCleanupsToDepth` to that loop's depth, then branches. `for-in` copies its label onto the desugared `for`. Rejected inside an `async fn` (`async_transform` guard, both compilers). Lockstep; gated by tc/cg parity + `tests/labeled_break.esk` |
...
| v0.8.0 | **Windows parity + language surface.** The full stdlib and both networking stacks run on a native Windows runner (`windows.yml`): mingw SEH exception personality (`__gxx_personality_seh0`), `<sysheap>` over `VirtualAlloc`, `<time>` over the Win32 clocks, `<threading>` over winpthreads, `<net>` over Winsock, and the **async** stack (`<eventloop>` WSAPoll reactor with fd-slots that grow to fit large `SOCKET` handles, `<net_async>` over `ioctlsocket`/`recv`/`send`/`WSAGetLastError`). Language: operator overloading (all operators, lockstep both compilers) and `match` exhaustiveness on payload-less enums. | ✅ |
| v0.9.0 | **Labeled `break`/`continue` + global-init fix.** A loop can carry a leading label (`outer: for (...)`) so `break outer` / `continue outer` target it from a nested loop, running the same defer cleanups; rejected inside `async` and inside a `defer` body. Lockstep both compilers (loops-only frame stack, separate from the switch-shared break-target). Fixed: global (and `static`-local) array initializers were silently zero-filled (only scalars kept their value); the constant folder now builds the array constant with C-style zero-fill and nested-array support, both compilers. | ✅ |
| v0.9.1 | **Correctness campaign (latent-bug hunt).** Fixed lockstep in both compilers: constant-initializer folding (64-bit literals, `struct`/`const`/`enum`/`sizeof`/`~`/`!`/`string`/hex globals; `static` accepts const forms; over-full array rejected), C signed/unsigned rules (`fptoui` for float→unsigned, `>>` follows the left operand, mixed-rank div/cmp), `switch`-`break` cleanup depth + `*Struct` bitfield write (C++), scientific-notation float literals, `--safe` slice bounds `0<=lo<=hi<=len`, and self-host async await typing + a clean error for await in a condition/larger-expr. Known remaining (invalid-program handling, emit bad IR vs a clean error): self-host method-call arg type-checking, and generic-monomorphization arg re-check. | ✅ |
